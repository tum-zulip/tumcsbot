...
class Move(PluginCommandMixin, Plugin):
    ...
    description = cleandoc(
        ...
        `destination` is parsed according to the following cases:
          - `_name` → `_name`, no topic
          - `#**_name**` → `_name`, no topic
          - `#**_name>topic**` → `_name`, `topic`
        ...
        **Note**:
        ...
        s!
        - This command does not work if the destination  name \
    ...
    def handle_message(self, message: dict[str, Any]) -> Response | Iterable[Response]:
        ...
        if "_id" not in message:
            return Response.build_message(message, "Error: not in a .")
        ...
        dest: tuple[str, str | None] | None = Regex.get__and_topic_name(
            " ".join(args.dest)
        )
    ...
    def _move(
        ...
        # Get destination  id.
        result = self.client.get__id(dest_)
        if result["result"] != "success":
            return Response.error(message)
        dest__id: int = result["_id"]
        ...
        # Get current  name.
        _name: str | None = self.client.get__name(_id)
        from_loc: str = (
            (_name if _name is not None else "unknown") + ">" + topic
        )
        to_loc: str = (
            dest_ + ">" + (dest_topic if dest_topic is not None else topic)
        )
